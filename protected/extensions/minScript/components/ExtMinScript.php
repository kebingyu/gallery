...
class ExtMinScript extends CClientScript {
	...
	/**
	 * @var string ID of the cache application component that is used by minScript. Set to false to disable
	 * caching. Defaults to "cache".
	 * @since 2.1
	 */
	...
	/**
	 * 
	 * @var string CDN prefix
	 */
	public $cdn = '';
	
	/**
	 * 
	 * @var string any string to bust the cache
	 */
	public $cacheBuster = '';
	 ...
	 * set to false, files will be checked on every request. Defaults to 600 seconds.
	 * @since 2.1
	 */
	public $minScriptLmCache = 600;
	...
	public function init() {
		...
		if ($this -> minScriptCacheId !== false && ($minScriptCache = Yii::app() -> getComponent($this -> minScriptCacheId)) !== null) {
			$this -> _minScriptCache = $minScriptCache;
		} else {
			Yii::app() -> setComponents(array('minScriptCache' => array('class' => 'system.caching.CDummyCache')), false);
	...
	public function minScriptGetLm($files, $log = false) {
		$files = (array)$files;
		$lmId = md5(Yii::app() -> id . 'minScriptLm' . serialize($files));
	...
	public function minScriptCreateGroup($files) {
		...
		$groupFile = Yii::app() -> runtimePath . '/minScript/groups/' . md5(Yii::app() -> id . 'minScriptGroup' . $filesSerialized);
		...
		if(!empty($this->cdn)){
			$return = $this->cdn .Yii::app() -> createUrl($this -> minScriptControllerId . '/serve', $params).$this->cacheBuster;
		}else{
			$return = Yii::app() -> createUrl($this -> minScriptControllerId . '/serve', $params).$this->cacheBuster;
		}
		return $return;
		...
		foreach (array_keys($files) as $key) {
			...
			foreach ($urls as $url) {
				if ($type == 'scripts') {
					...
					$this -> cssFiles[$url] = array_shift(explode('minScriptCssSort', $key));
				}
			}
		}
	}
