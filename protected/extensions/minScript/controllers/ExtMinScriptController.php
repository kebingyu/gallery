...
class ExtYiiMinCache {
	...
	public function store($id, $data) {
		return $this -> _yiiCache -> set(md5(Yii::app() -> id . 'minScriptMinifyCache' . $id), "{$_SERVER['REQUEST_TIME']}|{$data}", $this -> _exp);
	}
	...
	protected function _fetch($id) {
		...
		$ret = $this -> _yiiCache -> get(md5(Yii::app() -> id . 'minScriptMinifyCache' . $id));
		if (false === $ret) {
			$this -> _id = null;
			return false;
		}
